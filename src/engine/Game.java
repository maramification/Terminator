...
public class Game {
	...
	public void targetCity(Army army, String targetName) {
		int x = -1;
		for(int i = 0; i < this.player.getControlledArmies().size(); i++) {
			//System.out.println(army);
			if(this.getPlayer().getControlledArmies().get(i).getCurrentLocation().equalsIgnoreCase(army.getCurrentLocation())) {
		...
		String from = this.player.getControlledArmies().get(x).getCurrentLocation();
		if (this.player.getControlledArmies().get(x).getCurrentLocation().equals("onRoad"))
			from = army.getTarget();
		for (Distance d : distances) {
			if ((d.getFrom().equals(from) || d.getFrom().equals(targetName))
					&& (d.getTo().equals(from) || d.getTo().equals(targetName))) {
				this.player.getControlledArmies().get(x).setTarget(targetName);
				int distance = d.getDistance();
				if (this.player.getControlledArmies().get(x).getCurrentLocation().equals("onRoad"))
					distance += this.player.getControlledArmies().get(x).getDistancetoTarget();
				this.player.getControlledArmies().get(x).setDistancetoTarget(distance);
			//	System.out.println(army.getDistancetoTarget());
			}
		}
	...
	public void endTurn() {
		...
		for (Army a : player.getControlledArmies()) {
			...
			//System.out.println(a.getTarget() + a.getCurrentStatus());
